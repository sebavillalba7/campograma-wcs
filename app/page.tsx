...
function UploadSection({ matches, onCreated, onOpen, onDelete }: { matches: MatchRecord[]; onCreated: (m: MatchRecord) => void; onOpen: (m: MatchRecord) => void; onDelete: (id: string) => void }) {
  ...
  return <section className="page uploadPage"><div className="intro"><span className="eyebrow">NUEVO ANÁLISIS</span><h1>Del GPS al partido, en un solo flujo.</h1><p>Cargá el resumen por jugador/minuto y los CSV posicionales exportados desde OpenField. La app sincroniza, normaliza el campo y guarda el partido en este navegador.</p></div>
    ...
    <div className="saved"><div><span className="eyebrow">BIBLIOTECA LOCAL</span><h2>Partidos guardados</h2></div>{matches.length === 0 ? <div className="empty">Todavía no hay partidos guardados en este dispositivo.</div> : <div className="matchGrid">{matches.map(m => <article key={m.id}><span>{new Date(m.date + "T12:00:00").toLocaleDateString("es-AR")}</span><h3>{m.name}</h3><p>{m.athletes.length} GPS · {m.metrics.length} registros</p><div><button onClick={() => onOpen(m)}>Abrir análisis</button><button className="iconBtn" aria-label="Eliminar" onClick={() => onDelete(m.id)}><Trash2 /></button></div></article>)}</div>}</div>
  </section>;
}
