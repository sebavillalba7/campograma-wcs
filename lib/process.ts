import { keyOf, num, parseCsv } from "./csv";
...
function gpsAthlete(file: File, rows: ReturnType<typeof parseCsv>, warnings: string[]): Athlete | null {
  ...
  const rawName = String(athleteK && namedRow ? namedRow[athleteK] : file.name);
  ...
  const points: Point[] = local.map(p => ({ t: Math.max(0, Math.round(p.abs - t0)), x: p.east, y: p.north, speed: p.speed, acc: p.acc, quality: p.quality }));
...
function summaryMetrics(rows: ReturnType<typeof parseCsv>): MinuteMetric[] {
  ...
  if (!athleteK) return [];
  return rows.map((r, i) => {
    const athlete = String(r[athleteK] || "Jugador");
    const distance = num(distK ? r[distK] : 0);
    return { minute: Math.max(1, Math.round(num(minuteK ? r[minuteK] : i + 1))), athlete: label(athlete), position: athletePosition(athlete), distance, mtsMin: num(rateK ? r[rateK] : distance), hs19: num(h19K ? r[h19K] : 0), hs24: num(h24K ? r[h24K] : 0), ad: num(accK ? r[accK] : 0), maxSpeed: num(maxK ? r[maxK] : 0) };
  });
}
...
export async function processFiles(summary: File, gpsFiles: File[], meta: { opponent: string; date: string; sources: string[] }): Promise<MatchRecord> {
  ...
  const summaryRows = parseCsv(await summary.text());
  const athletes = (await Promise.all(gpsFiles.map(async f => gpsAthlete(f, parseCsv(await f.text()), warnings)))).filter(Boolean) as Athlete[];
